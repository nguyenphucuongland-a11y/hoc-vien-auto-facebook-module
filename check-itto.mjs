 ...
 * Kiểm: đủ 4 mục I-T-T-O · liệt kê secret/variable · MỌI script (.js/.mjs) khai trong itto CÓ TỒN TẠI
 * (bắt lỗi bàn giao thiếu file). Exit ≠ 0 nếu còn lỗi ⇒ cổng chốt trước khi chạy thật.
 *
...
// MỌI script khai trong itto (node <path>.js|.mjs) phải tồn tại trên đĩa
const paths = [...new Set([...text.matchAll(/node\s+([^\s"']+\.m?js)/g)].map((m) => m[1]))];
